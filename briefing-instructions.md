...
Write briefing in Traditional Chinese. Include date header. Taiwan section with alert level, foreign net, trust net, TSMC data, 0050 data, close, volume, top buyer and seller. Medium and full modes add 5-day trend table, top5 buy and sell lists, trust top buyer, hedge ratio. Full mode adds search context. After Taiwan section, add Entry Signal section: read 進場訊號 field (訊號 label, 分數 score/10, 依據 reasons array) and 訊號連續天數 (if ≥2, note "連續N天"). Display as: signal label in bold, score/10, streak if any, reasons as bullet list, one-line action advice. If 分歧信號 exists, add one line: "現貨×期貨分歧：[狀態]（現貨[買/賣超]，期貨[多/空]方）" — note that 分歧警戒 (spot buy + futures short) means hedging and warrants extra caution even with bullish spot flow. Global section with sp500 vix gold sox us10y usdtwd and changes. Trump section: short mode one line no major news. Analysis mode write full structured report with 4 sub-sections:
