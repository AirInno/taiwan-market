...
|-------------------------------|-----------------------|--------------------------------------|
| briefings/TW_DATE-tw.md       | Always                | This agent (Step 8)                  |
| briefings/TRUMP_DATE-trump.md | Always                | This agent (Step 8)                  |
...
**TW_DATE 和 TRUMP_DATE 是兩個獨立日期，2026/07/03 起拆分（見 STEP 8 說明）：**
- `TW_DATE` = 股票最新交易日（data-latest.json 最後一筆的 date 欄位）。台股收盤後才有資料，
  Agent 若在早上執行、當天尚未收盤，TW_DATE 就是前一個交易日，這是刻意設計，防止假重複報告。
- `TRUMP_DATE` = Agent 執行當下的台灣真實日曆日期（時鐘算出來的，不受股市影響）。川普新聞是連續性的，
  不像股票有「收盤」這種資料就緒的門檻，Agent 執行當下抓到的新聞就是「今天」的新聞，所以用真實時鐘日期
  最準確，不需要跟著 TW_DATE 慢一天。
- 這兩個日期在多數交易日會相同，但在「早上執行、股票還沒收盤」的情況下會不同（TW_DATE 較舊、
  TRUMP_DATE 是今天）——這是正常現象，不是 bug。
...
`[YYYY-MM-DD]` 與 `搜尋日期` 一律填入 **TRUMP_DATE**（Agent 執行當下的台灣真實日曆日期，見上方
「Output files」說明；轉成 YYYY-MM-DD 格式，例如 TRUMP_DATE=20260703 → 2026-07-03），
**不是** TW_DATE（股票交易日）。這是 2026/07/03 起的拆分設計，讓川普動態的日期標籤永遠對齊
Agent 實際搜尋的當下，不再套用股票那邊可能慢一天的舊日期。

```
# 🇺🇸 川普每日動態簡報 [YYYY-MM-DD from TRUMP_DATE]

搜尋日期：[TRUMP_DATE as YYYY-MM-DD]｜RSS 來源：[SOURCE]（[N] 則）
...
# 1b. 決定兩個獨立日期（2026/07/03 起拆分，見上方「Output files」說明）。

# 1b-i. TW_DATE — 「這份簡報所分析的最新交易日」，不可用時鐘/UTC 推算。
...
#       今天的股票報告會在明天早上資料就緒後、才以今天日期產生。）
TW_DATE=$(python3 -c "import json;print(json.load(open('/tmp/tw-repo/data-latest.json',encoding='utf-8'))[-1]['date'])")
echo "台股簡報日期（=最新交易日）: $TW_DATE"   # 例如 20260625

# 1b-ii. TRUMP_DATE — Agent 執行當下的「台灣真實日曆日期」，用時鐘算，不看股票資料。
#     川普新聞沒有「收盤」這種資料就緒門檻，Agent 執行當下抓到的新聞就是「今天」的新聞，
#     用真實時鐘日期最準確，不需要也不應該跟著 TW_DATE 慢一天。
#     用 UTC+8 手動換算，不依賴系統 tzdata（Asia/Taipei 可能未安裝），邊界已實測正確。
TRUMP_DATE=$(date -u -d "@$(( $(date -u +%s) + 28800 ))" +%Y%m%d)
echo "川普簡報日期（=執行當下台灣日曆日）: $TRUMP_DATE"

#     ↑ 台股簡報檔名/標題一律用 $TW_DATE；川普簡報檔名/標題一律用 $TRUMP_DATE。
#       多數交易日兩者相同，早上執行且股票未收盤時會不同（TW_DATE 較舊），這是正常現象。

# 2. Write files
# Write Taiwan briefing to /tmp/tw-repo/briefings/${TW_DATE}-tw.md
# Write Trump briefing to /tmp/tw-repo/briefings/${TRUMP_DATE}-trump.md

# 3. Commit and push
cd /tmp/tw-repo
git config user.email "agent@briefing.bot"
git config user.name "Briefing Agent"
git add briefings/${TW_DATE}-tw.md briefings/${TRUMP_DATE}-trump.md
git commit -m "briefing: 台股簡報 ${TW_DATE} + 川普簡報 ${TRUMP_DATE}"
...
Files to save:
- `briefings/${TW_DATE}-tw.md` — **always**（${TW_DATE} = STEP 8 讀 `data-latest.json` 最新一筆的 `date` 欄位，**不是**用系統時鐘/UTC 推算「今天」——這是刻意設計，標籤永遠對齊資料交易日，見上方 1b-i 說明）
- `briefings/${TRUMP_DATE}-trump.md` — **always**（${TRUMP_DATE} = Agent 執行當下的台灣真實日曆日期，**不是** TW_DATE——2026/07/03 起改為獨立日期，見上方 1b-ii 說明與「Output files」表格下方的拆分原因）
